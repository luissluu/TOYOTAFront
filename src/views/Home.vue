<template>
    ...
    <div v-if="ordenes.length" class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div
                v-for="orden in ordenes.slice(0, 4)"
                :key="orden.orden_id"
                class="bg-gradient-to-br from-gray-800 via-gray-900 to-gray-800 border border-gray-700 rounded-2xl shadow-lg p-6 transition-transform hover:scale-105 hover:shadow-2xl"
            >
                <div class="flex items-center justify-between mb-2">
                    ...
                    <span
                        v-if="(orden.estado || '').toLowerCase().trim() === 'finalizada'"
                        class="px-3 py-1 rounded-full bg-green-500/20 text-green-400 text-xs font-bold flex items-center gap-1"
                    >
                ...
                <div v-for="servicio in orden.detalles" :key="servicio.detalle_id" class="mb-4">
                    <div class="text-white font-medium mb-2 flex items-center">
                    ...
                    <div class="flex items-center gap-4">
                        <div v-for="(step, idx) in steps" :key="step.key" class="flex items-center">
                            <div :class="[
                                'w-8 h-8 flex items-center justify-center rounded-full border-2 font-bold transition-all duration-300',
                                getStepClass(mapEstadoStepper(servicio.estado), step.key)
                            ]">
                                <span class="text-lg">{{ step.icon }}</span>
                            </div>
                            <span class="ml-2 text-xs font-semibold transition-all duration-300" :class="getStepTextClass(mapEstadoStepper(servicio.estado), step.key)">
                                {{ step.label }}
                            </span>
                            <span v-if="idx < steps.length - 1" class="w-6 h-1 bg-gray-600 mx-2 rounded transition-all duration-300"></span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div v-if="ordenes.length > 4" class="flex justify-center mt-6">
